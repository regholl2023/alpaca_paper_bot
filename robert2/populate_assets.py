...
try:
    ...
    cursor.execute("""
        SELECT id, symbol, name, exchange FROM assets
    """)
    ...
    for asset in assets:
        try:
             if asset.status == 'active' and asset.tradable and asset.symbol not in symbols and asset.exchange != 'CRYPTO':
                    ...
                    cursor.execute("INSERT OR IGNORE INTO assets (symbol, name, exchange) VALUES (?, ?, ?)", (asset.symbol, asset.name, asset.exchange))
